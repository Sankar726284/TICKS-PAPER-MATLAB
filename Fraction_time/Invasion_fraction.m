...
Map1=invasionmap_det;
cutoff_time1=0:max(max(Map1));
Invaded_patch=zeros(numel(cutoff_time1),1);
Invaded_fraction1=zeros(numel(cutoff_time1),1);
fraction_map=zeros(px,py);
for jj=1:numel(cutoff_time1)
 for i = 1:px
   for j = 1:py
      if  invasionmap_det(i,j)<=cutoff_time1(jj)
        fraction_map(i,j)=1;
 ...
 Invaded_fraction1(jj)=Invaded_patch(jj)/patch;

end
for i=1:min(cutoff_time1)-1
    Invaded_fraction1(i)=0;
end



figure(1)
h1=plot(Invaded_fraction1,'LineWidth',2,'color','k');

hold on


w=5;%change it according to your needs. We used 200
...
Map2=round(mean(invasion_sto'));
invasionmap_sto=zeros(px,py);
 for i = 1:px
   for j = 1:py
       invasionmap_sto(i,j)= Map2((j-1)*px+i);
   end
 end

cutoff_time2=0:max(max(invasionmap_sto));
Invaded_patch2=zeros(numel(cutoff_time2),1);
Invaded_fraction2=zeros(numel(cutoff_time2),1);
fraction_map2=zeros(px,py);
for jj=1:numel(cutoff_time2)
 for i = 1:px
   for j = 1:py
      if  invasionmap_sto(i,j)<=cutoff_time2(jj)
        fraction_map2(i,j)=1;
      else fraction_map2(i,j)=0;
      end
   end
   end
 Invaded_patch2(jj)=sum(fraction_map2,'all');
 Invaded_fraction2(jj)=Invaded_patch2(jj)/patch;

end
for i=1:min(cutoff_time1)-1
    Invaded_fraction2(i)=0;
end

h2=plot(Invaded_fraction2,'LineWidth',2,'color','g');
...
legend([h1 h2],{'Deterministic','Stochastic'});
xticks(0:30:240);
xlim([0 240])
